...
import { AlertTriangle, ArrowLeft, Calendar, RotateCw } from "lucide-react";

/**
 * Catch-all error boundary for the App Router. Receives the thrown error +
 * a `reset()` that re-renders the current segment.
 */
export default function GlobalError({
  ...
  useEffect(() => {
    // Log to console for now; production would forward to Sentry/Logflare.
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-[#2D4EF5] flex items-center justify-center px-4 py-10">
      ...
      <main className="relative w-full max-w-md">
        ...
        <div className="rounded-[28px] bg-white px-8 py-11 sm:px-10 shadow-[0_30px_80px_-20px_rgba(15,23,42,0.35)] text-center">
          <div className="w-14 h-14 rounded-full bg-red-50 text-red-600 grid place-items-center mx-auto">
            <AlertTriangle size={26} strokeWidth={2} />
          </div>
          <h1 className="mt-4 text-2xl font-bold tracking-tighter text-ink-1">
            เกิดข้อผิดพลาด
          </h1>
          <p className="mt-2 text-sm text-ink-3">
            ระบบมีปัญหาขณะโหลดหน้านี้ ลองโหลดใหม่อีกครั้ง
            หรือกลับไปหน้าหลัก
          </p>

          {error.digest && (
            <p className="mt-4 text-[11px] text-ink-3 font-mono">
              ref: {error.digest}
            </p>
          )}
